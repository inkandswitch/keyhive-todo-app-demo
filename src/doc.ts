import { AutomergeUrl, DocHandle } from "@automerge/react/slim";
import {
  Access,
  ChangeRef,
  DocumentId,
...
import { Phonebook } from "@automerge/rootstock-identity";
...
export async function generateDoc<T>(
  ...
  changePhonebook((d: Phonebook) => {
    d.ids[docHandle.url as string] = keyhiveDoc.doc_id.toBytes();
    d.groups[docHandle.url as string] = g.id.toBytes();
  });

  // Add members after the identities doc is updated with group info
  changePhonebook(async (identitiesDoc: Phonebook) => {
