...
using [Automerge](https://automerge.org).
...
Clone the project, install its dependencies, and run `pnpm dev` to start the
local dev server.
...
You'll notice the URL change to append a hash with an Automerge document ID,
e.g.:

`http://localhost:5173/#automerge:8SEjaEBFDZr5n4HzGQ312TWfhoq`

Open the same URL (including the document ID) in another tab or another browser
to see each client's changes synchronize with all other active clients.
